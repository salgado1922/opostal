...
function GuidePage() {
  const g = Route.useLoaderData() as GuideRow;
  const c = g.content;

  return (
    <main className="bg-twilight-radial min-h-screen">
      {/* Hero */}
      <header className="relative flex min-h-[62vh] items-end overflow-hidden md:min-h-[78vh]">
        {g.hero_url && (
          <SmartImage
            src={g.hero_url}
            alt={g.hero_alt || `${g.city || g.title} — foto de capa`}
            priority
            sizes="100vw"
        ...
        <div className="relative mx-auto w-full max-w-5xl px-5 pb-12 md:px-12 md:pb-20">
          <p className="mb-3 text-[11px] uppercase tracking-[0.3em] text-gold/85">
            {[g.country, g.duration].filter(Boolean).join(" · ")}
          </p>
          <h1 className="font-serif text-4xl leading-tight md:text-6xl">
            <span className="text-gradient-gold">{g.title || g.city}</span>
          </h1>
          {g.intro && (
      ...
      <div className="px-5 pb-4 md:px-12">
        <div className="mx-auto max-w-5xl">
          <CustomItineraryCTA city={g.city || g.title} />
        </div>
      </div>

      <div className="flex justify-center pb-10">
        <FinalStamp code={(g.city || g.title || "postal").slice(0, 3).toUpperCase()} />
      </div>

      <SiteFooter city={g.city || g.title} farewell={c.farewell || undefined} />
    </main>
  );
}
